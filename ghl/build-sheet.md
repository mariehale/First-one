...
2. **Won = the sale closed, not the job finished.** An opportunity is marked **Won** the moment the client books (waiver signed, invoice sent) — at the **Booked - Waiver Signed** stage. A project can run for months; the sales cycle ended at booking, and waiting until the final visit (or the referral ask) would make every sales-cycle and win-rate report wrong. The stages after booking (In Progress → Final Visit → Referral Asked) are delivery tracking; the opportunity keeps its Won status while it moves through them. Workflow K sets Won automatically.
3. Review requesting/tracking doesn't belong in the pipeline either — it's a time-based nudge sequence with no natural "stage," so it's a **workflow** (see §7, Workflow I), not stages.
...
Each opportunity in this pipeline = one client engagement (matches one row in `tracker/clients.csv`). When a lead doesn't book, or a job falls through, mark the opportunity's status **Lost** (or **Abandoned** if it just went cold) — don't move it to a stage. It's marked **Won** automatically the moment it reaches **Booked - Waiver Signed** (Workflow K) — the sales cycle ends at booking, not when the project ends. The later stages just track delivery, and the `past-client` tag covers segmentation once the job wraps. Review requesting runs entirely in Workflow I, independent of the pipeline.
...
> When a contact is tagged final-visit-sent, wait 4 days, then send: "Honey, you deserve more — and so does anyone you know who's drowning in their own clutter. If a friend, a family member, or anyone comes to mind, we'd love an introduction. Refer someone and you both get $20 off, plus a spot in the House of Honey." Then add the tag referral-asked and move their opportunity to the Referral Asked stage. Do not change the opportunity's status — it was already marked Won at booking (Workflow K).
...
> 60 days after a contact is tagged final-visit-sent (not 60 days after Won — Won happens at booking, and a project can run for months), send: "It's been about two months since your reset — how's the system holding up, Honey? We offer a quick maintenance visit (1–2 hrs) to reset anything that's drifted. Want to grab a spot?"
...
### K. Booked → Won + Charge + Payment Reminders (build after §5; replaces "charge at the end")
> When an opportunity in the Home Reset Pipeline moves to the Booked - Waiver Signed stage, immediately mark the opportunity's status as Won, add the tag waiver-signed, and send the contact an invoice for their package by email and SMS with this message: "You're officially booked, Honey! Here's your invoice to lock in your spot — Quick Refresh and Room Reset are paid in full to book; hourly projects pay the 3-hour minimum now and we invoice the rest after each visit." Wait 2 days. If the invoice is not paid, send a friendly reminder: "Quick nudge, Honey — your invoice is still open, and paying it locks in your date. Here's the link again." Wait 2 more days. If still not paid, send one more reminder and create a task for Marie due in 1 day: "[contact name]'s booking invoice is unpaid after 4 days and two reminders — call them before the first session." When the invoice is paid at any point, stop sending reminders.
